...
def print_indices(items):
    ...
    items_tulips = list(enumerate(items)) #create a list of tulips, each tulip is a pair of item's index and item value
    
    items_with_indices = [str(item[0]) + ' ' + item[1] for item in items_tulips] # list of strings, each string is from tulip
    
    print("\n".join(items_with_indices)) # printing
...
def foods_in_common(foods1, foods2):
    ...
    **NOTE**: for this problem, you're welcome to use any of the
    ...
    """
    foods1_set = set(foods1) # set from 1st list
    foods2_set = set(foods2) # set from 2nd list
    
    common_foods = sorted(list(foods1_set & foods2_set)) # sorted intersection of sets from above
...
def every_other_item(items):
    ...
    """
    i = 0 #counter
    every_other_items = [] # new list for every other element
    
    while 2*i < len(items):
        every_other_items[i:i] = [items[2*i]]
        i = i + 1

    print every_other_items
...
def largest_n_items(items, n):
    ...
    separately::
        ...
        [3, 3]
    """
    sorted_items = sorted(items) # sorted list

    #return slice of sorted list
    return sorted_items[len(sorted_items) - n:]
